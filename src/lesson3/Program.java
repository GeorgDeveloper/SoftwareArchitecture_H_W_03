...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        Harvester kamaz = new Harvester("KAMAZ", "CITY", Color.BLACK, Diesel);

        RefuelingStation lukOil = new RefuelingStation();

        kamaz.setRefuelingStation(lukOil);

        kamaz.fuel();

        CarWash drop = new CarWash();

        kamaz.setCarWash(drop);
        kamaz.wipMirrors();
        kamaz.wipHeadlights();
        kamaz.wipWindshield();

        SoprtCar vestaSport = new SoprtCar("Lada", "VestaSport", Color.BLUE, Gasoline);
        RefuelingStationV2 rosNeft = new RefuelingStationV2();
        vestaSport.setRefuelingStation(rosNeft);
        vestaSport.fuel();

        CarWash moika = new CarWash();
        vestaSport.setCarWash(moika);
        vestaSport.wipHeadlights();
